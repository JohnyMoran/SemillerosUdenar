...
class ProyectosController extends Controller
{
    ...
    {
        $request->validate([
            'codigo' => 'required|unique:proyectos,codigo,' . $proyecto->id,
            'titulo' => 'required|unique:proyectos,titulo,' . $proyecto->id,
            'integrantes.*' => 'integer',
            'tipo_proyecto' => 'required|in:investigacion,innovacion,emprendimiento',
            'estado' => 'required|in:propuesta,en_curso,terminado',
            'fecha_inicio' => 'required|date',
            'fecha_finalizacion' => 'required|date',
            'archivo_propuesta' => 'file|mimes:pdf',
            'archivo_proyecto_final' => 'file|mimes:pdf',
        ]);
        ...
        $proyecto->integrantes = $integrantesJson;

        if ($request->hasFile('archivo_propuesta')) {
            $archivoPropuesta = $request->file('archivo_propuesta');
            $rutaArchivoPropuesta = $archivoPropuesta->store('archivos_propuesta', 'public');
            $proyecto->archivo_propuesta = $rutaArchivoPropuesta;
        }

        if ($request->hasFile('archivo_proyecto_final')) {
            $archivoProyectoFinal = $request->file('archivo_proyecto_final');
            $rutaArchivoProyectoFinal = $archivoProyectoFinal->store('archivos_proyecto_final', 'public');
            $proyecto->archivo_proyecto_final = $rutaArchivoProyectoFinal;
        }

        $proyecto->save();

        return redirect()->route('admin.proyectos.index')
            ->with('success', 'El proyecto se ha actualizado exitosamente.');
    }
    ...
    public function generateProyectosPDF()
    {
        $proyectos = Proyecto::all();
        $pdf = new Dompdf();
        
        $html = view('admin.reports.proyectos_pdf', compact('proyectos'))->render();
