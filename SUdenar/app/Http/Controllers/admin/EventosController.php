...
class EventosController extends Controller
{
    ...
    {
        $request->validate([
            'codigo' => 'required|unique:eventos,codigo,' . $evento->id,
            'nombre' => 'required',
            'descripcion' => 'required',
            'fecha_inicio' => 'required|date',
            'fecha_fin' => 'required|date|after_or_equal:fecha_inicio',
        ...
        return redirect()->route('admin.eventos.index')
            ->with('success', 'El evento se ha actualizado exitosamente.');
    }

    public function generateEventosPDF()
    {
        $eventos = Evento::all(); // Obtener la lista de eventos
        $pdf = new Dompdf();
        $pdf->loadView('eventos-report', compact('eventos'));
